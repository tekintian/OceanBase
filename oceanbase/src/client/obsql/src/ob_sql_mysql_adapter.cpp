...
static void clear_connection_(ObSQLMySQL *ob_mysql)
{
  ...
  if (ob_mysql->rconn_  != ob_mysql->wconn_)
  {
    ...
    {
      release_conn(ob_mysql->rconn_);
      ob_mysql->rconn_ = NULL;
    }

    if (NULL != ob_mysql->wconn_)
    {
      release_conn(ob_mysql->wconn_);
...
MYSQL_STMT * STDCALL MYSQL_FUNC(mysql_stmt_init)(MYSQL_TYPE *mysql)
{
  ...
  if (NULL != mysql && OB_SQL_MAGIC == ((ObSQLMySQL*)mysql)->magic_)
  {
    ...
    if (NULL == (real_mysql = select_connection((ObSQLMySQL *)mysql, "select stmt", 11, NULL)))
    {
      TBSYS_LOG(ERROR, "select_connection fail, can not find a valid connection in %s", __FUNCTION__);
    }
    else
    {
      ret = CALL_REAL(mysql_stmt_init, real_mysql);
    }
  }
  else
  {
