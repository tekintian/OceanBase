...
namespace oceanbase
{
  ...
  namespace liboblog
  {
    ...
    int ObLogRouter::init(ObLogConfig &config, IObLogFilter *source, IObLogSchemaGetter *schema_getter)
    {
      ...
      else if (0 != table_record_map_.create(MAX_TABLE_NUM))
      {
        ...
        ObLogMutator *mutator, uint64_t *db_partition)
    {
            ...
            && OB_SUCCESS == (ret = mutator->get_mutator().next_cell()))
      {
        ...
        if (OB_SUCCESS != (ret = mutator->get_mutator_cell(&cell)) || NULL == cell)
        {
    ...
    void ObLogRouter::handle_with_stopflag(void *task, void *pdata, volatile bool &stop_flag)
    {
      ...
      if (skip_dirty_data_ && OB_INVALID_DATA == tmp_ret)
      {
        TBSYS_LOG(WARN, "Thread %ld: skip dirty data, mutator: TM=%ld, CP=%lu, is_heartbeat=%s",
            S2MQueueThread::get_thread_index(), mutator->get_mutate_timestamp(),
            mutator->get_log_id(), mutator->is_heartbeat() ? "true" : "false");

        tmp_ret = OB_SUCCESS;
      }
      else if (OB_SUCCESS != tmp_ret && OB_IN_STOP_STATE != tmp_ret)
      {
        TBSYS_LOG(ERROR, "Thread %ld aborted, add mutator fail, "
            "mutator: TM=%ld, CP=%lu, is_heartbeat=%s, ret=%d",
            S2MQueueThread::get_thread_index(), mutator->get_mutate_timestamp(),
            mutator->get_log_id(), mutator->is_heartbeat() ? "true" : "false", tmp_ret);
