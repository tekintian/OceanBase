...
namespace oceanbase
{
  namespace liboblog
  {
    ...
    class ObLogBinlogRecord : public IBinlogRecord
    {
      ...
      public:
        ObLogBinlogRecord() : data_(NULL), next_(NULL)
        {
      ...
      public:
        ...
        void setCheckpoint(unsigned file, unsigned offset) { BR_CALL(setCheckpoint, file, offset); }
         ...
         */
        unsigned getCheckpoint1() { BR_CALL_WITH_RETVAL(getCheckpoint1); }
        unsigned getCheckpoint2() { BR_CALL_WITH_RETVAL(getCheckpoint2); }

        unsigned getFileNameOffset() { BR_CALL_WITH_RETVAL(getFileNameOffset); }
        unsigned getFileOffset() { BR_CALL_WITH_RETVAL(getFileOffset); }
      ...
      public:
         ...
         * 清除所有内容以便重用, 不释放用户内存
         */
        void clear() { BR_CALL(clear); next_ = NULL; setTableMeta(NULL); setRecordType(EUNKNOWN); }

        /**
         * 清除所有内容，包括用户内存
         */
        void clearWithUserMemory() { BR_CALL(clearWithUserMemory); next_ = NULL; }
