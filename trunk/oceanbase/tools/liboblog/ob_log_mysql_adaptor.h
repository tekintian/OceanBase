...
namespace oceanbase
{
  namespace liboblog
  {
    ...
    class ObLogMysqlAdaptor
    {
      ...
      static const int64_t TC_INFO_MAP_SIZE = MAX_TABLE_NUM;
      static const int32_t MAX_MYSQL_FAIL_RETRY_TIMES = 100;

      public:
        /// @brief Class function used to get MySQL Adaptor based on specific schema
        ...
        static int get_mysql_adaptor(ObLogMysqlAdaptor *&mysql_adaptor,
      ...
      public:
        ...
        /// @retval OB_INVALID_DATA when rowkey does not match with schema
      ...
      private:
        ...
        /// @brief Update TCInfo of target table based on specific schema.
        /// @param table_id target table ID
        /// @param total_schema specific schema
        /// @retval OB_SUCCESS on success
        /// @retval OB_SCHEMA_ERROR when target schema is invalid and should be updated.
        /// @retval ! OB_SUCCESS on fail
        int update_tc_info_(uint64_t table_id, const ObLogSchema *total_schema);
        ...
        /// @brief Initialize mysql handle and statement: ObSQLMySQL and MYSQL_STMT
        /// @param[out] mysql returned ObSQLMySQL
        ...
        /// @retval ! OB_SUCCESS on fail
        int init_mysql_stmt_(ObSQLMySQL *&mysql, MYSQL_STMT *&stmt, const char *sql, int64_t sql_len);

        /// @brief The internal function to initialize mysql handle and statement: ObSQLMySQL and MYSQL_STMT
        /// @param[out] mysql returned ObSQLMySQL
        /// @param[out] stmt returned MYSQL_STMT
        /// @param[in] sql SQL statement
        /// @param[in] sql_len SQL statement length
        /// @retval OB_SUCCESS on success
        /// @retval OB_SCHEMA_ERROR when schema is invalid and failed to execute prepare statement
        /// @retval OB_NEED_RETRY when mysql_stmt_init() or mysql_stmt_prepare() fails because of unknown error
        /// @retval ! OB_SUCCESS on fail
        int init_mysql_stmt_internal_(ObSQLMySQL *&mysql, MYSQL_STMT *&stmt, const char *sql, int64_t sql_len);

        /// @brief Destroy mysql handle and statement: ObSQLMySQL and MYSQL_STMT
        /// @param mysql target ObSQLMySQL
        /// @param stmt MYSQL_STMT
        void destroy_mysql_stmt_(ObSQLMySQL *mysql, MYSQL_STMT *stmt);
        ...
        /// @retval OB_SUCCESS on success
        /// @retval ! OB_SUCCESS on fail
        int init_mysql_params_(MYSQL_BIND *&params, MYSQL_TIME *&tm_data, const TableSchemaContainer &tsc);

        /// @brief Destroy mysql parameters: MYSQL_BIND and MYSQL_TIME
        /// @param params target MYSQL_BIND
        /// @param tm_data target MYSQL_TIME
        void destroy_mysql_params_(MYSQL_BIND *params, MYSQL_TIME *tm_data);
      ...
      private:
        ...
        // Allocator
        common::FIFOAllocator allocator_;  ///< Allocator for heartbeat mutator
    };
  }
}
